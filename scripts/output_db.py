...
def fac_to_db_1(facility, date, facility_data):
    '''updates data from all files in a directory to a database'''
    ...
    c.execute("INSERT INTO " + table + " VALUES " + str(insert_tuple_1(facility,date, facility_data)))
    conn.commit()
    conn.close()
    return
...
def insert_tuple_1(facility,date, facility_data):
    '''Creates a tuple of facility data for insertion into a database'''
    f = facility_data[(facility,date)]
